...
int read_audio(const char *filename , vector<float> &out, int *sr, bool mono) {
    void *pWav = wav_read_open(filename);
    int format, channels, bits_per_sample;
    unsigned int data_length;
    int res = wav_get_header(pWav, &format, &channels, sr, &bits_per_sample, &data_length);
    if (!res) {
        printf("get ref header error: %s\n", filename);
        return -1;
    }

    int samples = data_length * 8 / bits_per_sample;
    vector<int16_t> tmp(samples);
    res = wav_read_data(pWav, reinterpret_cast<unsigned char *>(tmp.data()), data_length);
    if (res < 0) {
        printf("read wav file error: %s\n", filename);
        return -1;
    }
    vector<float> data(samples);
    transform(tmp.begin(), tmp.end(), data.begin(),
              [](int16_t a) {
                  return static_cast<float>(a) / 32768.f;
              });
    if (mono && channels == 2) {//合并双声道
        for (int i = 0; i < samples / 2; ++i) {
            out.push_back((data[i << 1] + data[(i << 1) + 1]) / 2);
        }
    } else {
